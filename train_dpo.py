...
def ensure_trl_tokenizer_compat(tokenizer: Any) -> Any:
    if not hasattr(tokenizer, "tokenizer") or getattr(tokenizer, "tokenizer", None) is not tokenizer:
        tokenizer_cls = tokenizer.__class__
        compat_cls = getattr(tokenizer_cls, "_trl_compat_cls", None)
        if compat_cls is None:
            class TrlCompatTokenizer(tokenizer_cls):
            ...
            compat_cls = TrlCompatTokenizer
            setattr(tokenizer_cls, "_trl_compat_cls", compat_cls)
        tokenizer.__class__ = compat_cls
    return tokenizer
